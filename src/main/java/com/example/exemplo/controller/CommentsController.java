...
public class CommentsController {
    ...
    public CommentsController(@Qualifier(STRATEEGIA_REST_TEMPLATE) RestTemplate restTemplate) { // @Qualifier serve para especificar qual @Bean importar. Como se fosse, chamar ele por um nome
        this.restTemplate = restTemplate;
        this.tokenStore = StrateegiaInMemoryTokenStore.getInstance();
    }

    @GetMapping("/comments")
    public ResponseEntity<SummaryResponse> getComments(
            @RequestParam(value = "divergencePointId") String divergencePointId,
            @RequestParam(value = "questionId") String questionId

    ) {

        String baseUrl = "/projects/v1/divergence-point/" + divergencePointId + "/question/" + questionId + "/comments";
        ...
        try {
            ...
            log.error("Erro na solicitação do conteudo do ponto", e);
